...
basePath = "Input_Data/DatasetsTFBSRegulonDB12.0/"
riFilePath = basePath+"RI_mapping_to_TFBS-HT/input/TF-RISet_v2.0.tsv"
'''chipexoPath = basePath + \
    "ChIP-exo/Dataset_author_files/ChIP-exo_v4.0(tf-gene_mapped)/"
chipseqPath = basePath + \
    "ChIP-seq/Dataset_author_files/ChIP-seq_v4.0(tf-gene_mapped)/"
dapseqPath = basePath + \
    "DAP-seq/Dataset_author_files/DAP-seq_v4.0(tf-gene_mapped)/"
gselexPath = basePath + \
    "gSELEX/Dataset_author_files/gSELEX_v4.0(tf-gene_mapped)/"
...
outputFilePath = basePath + \
    "RI_mapping_to_TFBS-HT/output/TFRIs_RDB12.0_Risv2.0_mapped.txt"
errorOutputFilePath = basePath + \
    "RI_mapping_to_TFBS-HT/output/Error_ClassConfwithoutHTdatasets_without_coords_v0.2_RDB12.0.txt"
        ...
        for collection in allCollections:
            ...
            for i, row in filteredMetadata.iterrows():
                ...
                if pd.notna(fileName):
                    ...
                    '''dfCurrentDataset = pd.read_excel(
                        currentDatasetPath, comment='#', header=0)'''
                    dfCurrentDataset = pd.read_csv(
                        currentDatasetPath, sep='\t', comment='#', header=0)
